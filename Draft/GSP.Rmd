...
It's important to cross check you data cleaning code. The common self-checking points are listed below:
...
"Sepal.Width*Sepal.Length" is the interaction term. Most of regression models are easy to fit. The following lists are some of known issues and concerns for common models (some are not specific to R): 

* Some GLMs, particularly Relative Risk regression also known as log-binomial regression (GLM with log link and binomial distribution model), have convergence problem [@Williamson_2013]. This is relate to parameter constraints at boundaries. For log-binomial regression, a package [*logbin*](https://cran.r-project.org/web/packages/logbin/logbin.pdf) was developed with advance convergence ability (use combinatorial EM to cycles through parameter space). For other GLMs, Bayesian models (i.e. using [*brms*](https://cran.r-project.org/web/packages/brms/vignettes/brms_overview.pdf)) can be used to overcome some of the convergence problems.
* Linear mixed effect models sometimes have convergence issues too. You can test performance of different optimizers, see example [here](http://svmiller.com/blog/2018/06/mixed-effects-models-optimizer-checks/). 
* Multinomial logistic regression model [*nnet::multinom*](nnet::multinom) requires predictors to be roughly scaled to [0,1] or the fit will be slow or may not converge at all. Normally this is fine, however when you have some large values (i.e. IRSAD scores which is around 1000), the model will stop working. Hence it will be better to standardise those predictors.
* Sometimes there will be an error message "Error: no valid set of coefficients has been found: please supply starting values", this means the model is having trouble to initialize the optimization and require you to supply a set of starting values for intercept and predictors. I normally run a model with reduced number of predictors and use the coefs from the reduced model as starting values. 
* Set seed for models require random sampling, i.e. bootstrap. 
* It will be better to use splines (i.e. [*ns*](https://www.rdocumentation.org/packages/splines/versions/3.6.2/topics/ns)) instead of square and cube terms to evaluate non-linear associations, see a review by @Perperoglou_2019.
* Different package will not necessary produce identical results, but they should generally agree with each other.  If not something is wrong: either a bug in your code or in the existing code libraries.
...
(2) Do not report p-value when hypothesis testing is meaningless.  A perfect example is a correlation coefficient $\rho=0.08$ with p-value< 0.001 (see example below). 
```{r}
set.seed(123)
a <- rnorm(1000)
b <- rnorm(1000)
...
(3) Include reference values. *tab_model* is currently having trouble to print out reference values. It is also difficult to configure custom settings and print LaTex table. The function below prints out linear regression model results and can be configured for different types of models and output requiements. 
...
The benefit of using a full programming language instead of a statistical package for analysis is that you have unlimited potential to extend your analysis, i.e. develop new methods, web scraping, generate interactive graphics, establish website, draft full paper etc.  There are some basic skills that could facilitate these advanced techniques, which require some attention. 


### Control flow 

The control flow in R is similar to other programming languages including choices (i.e. if/else, switch) and loops (i.e. for loop).  The for loop will probably be one of your most frequently used control flow in analysis as we frequently need to repeat operations. You can loop over a vector or a list. 
...
```
One thing to notice is that comparing with vectorized operations, using loop will slow down your code. This is a more severe issue for Python compared with R, using R with simple operations you probably will not notice any differences. However, when your work envolve with samplping, simulation or large datasets, the reduction in speed can be substentiall. 
...
This doesn't mean that you are not recommended to use loop. Actually compared with repetitively typing analysis code, using a function + loop is much more reliable and easy to maintain. A few rules to remember when using the loop are listed here:

* Use vectorized operations as much as possible 
* Always pre-define and fill the object and avoid grow the object (R automatically create copies of a object when you try to add undefined element(s), i.e. a new row). 
...
The operating environment is a slightly complicated concept to explain to non-programmers. Detailed description can be found in the [Advanced R by Hardley Wickham](https://adv-r.hadley.nz/environments.html). You are using the environment every time when you use R even you do not realize it. Essentially  you can think the environment as a name list of all the data, variables, libraries, functions and other objects. The name list links a name with a part of memory on your computer. Every name must be unique in an Environment. 
...
Some times there are confusions. See the example below:
...
Scoping rules are closely related to environment, it determines how values are assigned to free variables. An advanced introduction is provided in [R Programming for Data Science by Roger Peng](https://bookdown.org/rdpeng/rprogdatascience/scoping-rules-of-r.html). R uses lexical scoping which means that it will search the values of free variables or object are searched for in the environment in which the function was defined. The order of the r search start from the local environment first, next to the parent environments and then to the global environment. Although R does this searching all the time, you may not notice any issues unless the objects with the same name were defined from different environments, i.e. same variable names in the nested functions. The recommendation is try to avoid using the same name for different variables/objects. 
...
## Develop a "coding style" that is compaliable with others

## Avoid repeating

## Balance between function coding and hard coding
...
# Protocal for code check and reviewing 

## Is the statsitcal method appropriate

## Are the assumptions met or can be safely ignored? 
